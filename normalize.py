...
for meshname in tqdm(meshes):
    m = trimesh.load(src+'/untitled.glb',process=False, force='mesh')
    ...
    # scene.show('gl')
    _ = m.export(src+'/untitled_labeled_normalized.obj')

    ms = pymeshlab.MeshSet()
    ms.load_new_mesh(src+'/untitled_labeled_normalized.obj')
    ms.normalize_vertex_normals()
    ms.normalize_face_normals()
    ms.save_current_mesh(src+'/untitled_labeled_normalized.obj')